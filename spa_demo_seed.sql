...
-- Get the first auth user as owner
SELECT id INTO v_owner_id FROM auth.users LIMIT 1;
IF v_owner_id IS NULL THEN
    RAISE EXCEPTION 'No auth.users found. Please log in to Supabase first.';
END IF;
...
VALUES (gen_random_uuid(), 'Aura Med Spa Inc', v_owner_id, now())
RETURNING id INTO v_org_id;
RAISE NOTICE 'Created org_id: %', v_org_id;
...
VALUES (
    ...
    'Aura Med Spa', '456 Blossom Ave', 'Los Angeles', 'CA', '90210', 'US',
    'America/Los_Angeles', 'US', '+13104567890', now()
)
RETURNING id INTO v_clinic_id;
RAISE NOTICE 'Created clinic_id: %', v_clinic_id;

-- -----------------------------------------------------------------------
-- 3. Agent  (status omitted → uses DB default 'offline')
-- -----------------------------------------------------------------------
INSERT INTO agents (id, organization_id, clinic_id, name, default_language, created_at)
VALUES (gen_random_uuid(), v_org_id, v_clinic_id, 'Ava', 'en-US', now())
RETURNING id INTO v_agent_id;
RAISE NOTICE 'Created agent_id: %', v_agent_id;

-- -----------------------------------------------------------------------
-- 4. Agent settings
--    persona_tone valid values: friendly | professional | conversational | empathetic
-- -----------------------------------------------------------------------
INSERT INTO agent_settings (
    id, organization_id, agent_id,
    greeting_text, persona_tone,
    collect_insurance, emergency_triage_enabled, booking_confirmation_enabled,
    config_json, created_at
)
VALUES (
    gen_random_uuid(), v_org_id, v_agent_id,
...
-- -----------------------------------------------------------------------
-- 5. Phone number
--    provider enum valid values: twilio | telnyx | bandwidth | other
-- -----------------------------------------------------------------------
INSERT INTO phone_numbers (id, organization_id, clinic_id, agent_id, phone_number, phone_e164, label, created_at)
VALUES (
    gen_random_uuid(), v_org_id, v_clinic_id, v_agent_id,
    '+1 (310) 456-7890', '+13104567890', 'Main Spa Line', now()
);
RAISE NOTICE 'Phone number linked';

-- -----------------------------------------------------------------------
-- 6. Knowledge articles (20)
-- -----------------------------------------------------------------------

INSERT INTO knowledge_articles (id, organization_id, clinic_id, title, category, body, created_at)
VALUES (gen_random_uuid(), v_org_id, v_clinic_id,
